...
class TestFullIntegration(unittest.TestCase):
    ...
    def test_full_pipeline_10_seconds(self, MockPipeline, MockCamera):
        ...
        face.blendshapes = None
        face.transformation_matrix = None
    ...
    def test_multi_face_scenario(self, MockPipeline, MockCamera):
        ...
        f1 = MagicMock(spec=FaceDetection); f1.bbox = (0,0,10,10); f1.face_crop_299 = np.zeros((1,3,299,299), dtype=np.float32); f1.face_crop_raw = np.zeros((10,10,3), dtype=np.uint8); f1.landmarks = np.zeros((478,2)); f1.head_pose = (0.0, 0.0, 0.0); f1.is_frontal = True; f1.occlusion_score = 0.0; f1.blendshapes = None; f1.transformation_matrix = None
        f2 = MagicMock(spec=FaceDetection); f2.bbox = (20,20,10,10); f2.face_crop_299 = np.zeros((1,3,299,299), dtype=np.float32); f2.face_crop_raw = np.zeros((10,10,3), dtype=np.uint8); f2.landmarks = np.zeros((478,2)); f2.head_pose = (0.0, 0.0, 0.0); f2.is_frontal = True; f2.occlusion_score = 0.0; f2.blendshapes = None; f2.transformation_matrix = None
        ...
        with patch("onnxruntime.InferenceSession") as MockSession, \
            ...
            # First call is consumed by warmup in create_optimized_session
            MockSession.return_value.run.side_effect = [
                [np.array([[0.5, 0.5]], dtype=np.float32)], # Warmup (consumed by engine init)
            ...
            # On first frame: no blinks yet → liveness=FAIL
            # Face 1 (Real neural): REAL+FAIL+PASS → WAIT_BLINK
            # Face 2 (Fake neural): FAKE+FAIL+PASS → FAKE
            states = [r.state for r in result.face_results]
            # At least one face should be identified as FAKE
            self.assertIn("FAKE", states)
            # The states should differ (independent per-face tracking)
